  ...
  function updateData() {
    ...
    chrome.storage.local.get('matchesData', function(result) {
      ...
      matchList.innerHTML = matchesWithin24Hrs.map(match => {
        ...
        const eventName = match.event ? match.event : "Unknown Event";  // Retrieve event name from data
        const localTime = match.date === "Date not specified" ? "Live" : new Date(match.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        const matchLink = match.matchLink ? match.matchLink : "#";  // Retrieve match link from data
        
        return `
          <a href="${matchLink}" target="_blank" class="list-group-item list-group-item-dark"> <!-- Added anchor tag -->
            <div class="d-flex w-100 justify-content-between">
              <div>
                <div class="event-name">${eventName}</div> <!-- Event name -->
                <div class="team">
      ...
      // Initialize SimpleBar (remove the height setting)
